pub(crate) fn ground_track(message: &[u32]) -> Option<u32> {
    crate::decoder::flag_and_range_value(message, 45, 46, 52)
        .filter(|&f| f.0 == 1)
        .map(|v| (v.1 * 360) >> 7)
}
...
pub(crate) fn track_and_groundspeed(
    ...
    let sp_west = match crate::decoder::flag_and_range_value(message, 46, 47, 56) {
    ...
    let sp_south = match crate::decoder::flag_and_range_value(message, 57, 58, 67) {
...
pub(crate) fn heading(message: &[u32]) -> Option<u32> {
    crate::decoder::range_value(message, 47, 56)
}
