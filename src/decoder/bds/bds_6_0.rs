...
pub fn is_bds_6_0(message: &[u32]) -> Option<HeadingAndSpeed> {
    ...
    {
        let bds60 = HeadingAndSpeed::from_data(
        ...
        if bds60
            ...
            && bds60.mach_number.is_some_and(|x| (0.0..=1.0).contains(&x))
            && (bds60
                .barometric_altitude_rate
                .is_some_and(|x| (-6000..=6000).contains(&x))
                || bds60.barometric_altitude_rate.is_none())
            && (bds60
                .internal_vertical_velocity
                .is_some_and(|x| (-6000..=6000).contains(&x))
                || bds60.internal_vertical_velocity.is_none())
        {
            Some(bds60)
        } else {
            None
